 ...
 *     para Euros España  -> EUR
 *     para Pesos Argentinos -> ARS
 *     para reales Brasileños  -> BRL
 *     
...
public class funcioncambio {
	...
	public double valor(double monto,String monedaInternacional,String monedaLocal){
		double res=0;
		if(RangoNumero(monto) == false || EstaMoneda(monedaInternacional) == false || EstaMoneda(monedaLocal) == false || monedaInternacional.equals(null)|| monedaLocal.equals(null)) res=0; // retorna 0.0 si supera rango o no esta en el tipo de entrada de tipo moneda
		else{ 
		if(esDistinto(monedaInternacional, monedaLocal)) res=monto; // retorna el mismo valor si es el mismo tipo de moneda
		else{
		URL url;
			try {
